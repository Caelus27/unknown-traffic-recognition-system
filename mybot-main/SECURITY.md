...
If you discover a security vulnerability in mybot, please report it by:
...
**Recommendations:**
- Store API keys in `~/.mybot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.mybot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run mybot with a dedicated user account
...
- Keep authentication data in `~/.mybot/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for mybot and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install mybot-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash mybot
   sudo -u mybot mybot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.mybot
   chmod 600 ~/.mybot/config.json
   chmod 700 ~/.mybot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.mybot/logs/mybot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.mybot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.mybot/logs/mybot.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/mybot/security/advisories
- Release Notes: https://github.com/HKUDS/mybot/releases
