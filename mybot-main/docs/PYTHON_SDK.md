...
Use mybot programmatically — load config, run the agent, get results.
...
### `MyBot.from_config(config_path?, *, workspace?)`

Create a `MyBot` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.mybot/config.json`. |
...
```python
from mybot.agent import AgentHook, AgentHookContext
...
async def main():
    bot = MyBot.from_config(workspace="/my/project")
