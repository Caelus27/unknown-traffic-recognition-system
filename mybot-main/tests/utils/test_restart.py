...
def test_set_and_consume_restart_notice_env_roundtrip(monkeypatch):
    monkeypatch.delenv("MYBOT_RESTART_NOTIFY_CHANNEL", raising=False)
    monkeypatch.delenv("MYBOT_RESTART_NOTIFY_CHAT_ID", raising=False)
    monkeypatch.delenv("MYBOT_RESTART_STARTED_AT", raising=False)
    ...
    assert "MYBOT_RESTART_NOTIFY_CHANNEL" not in os.environ
    assert "MYBOT_RESTART_NOTIFY_CHAT_ID" not in os.environ
    assert "MYBOT_RESTART_STARTED_AT" not in os.environ


def test_format_restart_completed_message_with_elapsed(monkeypatch):
    monkeypatch.setattr("mybot.utils.restart.time.time", lambda: 102.0)
