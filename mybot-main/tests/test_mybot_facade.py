"""Tests for the MyBot programmatic facade."""
...
from mybot.mybot import MyBot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        MyBot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = MyBot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("mybot.config.loader.load_config") as mock_load, \
         patch("mybot.mybot._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = MyBot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from mybot.config.schema import Config
    from mybot.mybot import _make_provider
...
def test_import_from_top_level():
    from mybot import MyBot as N, RunResult as R
    assert N is MyBot
