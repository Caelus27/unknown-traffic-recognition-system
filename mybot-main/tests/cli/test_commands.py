...
from mybot.cli.commands import _make_provider, app
from mybot.config.schema import Config
from mybot.providers.openai_codex_provider import _strip_model_prefix
...
def test_console_entry_invokes_typer_app(monkeypatch):
    called = False
    ...
    monkeypatch.setattr("mybot.cli.commands.app", fake_app)

    from mybot.cli.commands import cli
...
def mock_paths():
    ...
    with patch("mybot.config.loader.get_config_path") as mock_cp, \
         ...
         patch("mybot.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from mybot.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "mybot.cli.onboard.run_onboard",
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'mybot agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"mybot serve --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("mybot.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from mybot.cli.commands import _make_provider
    from mybot.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("mybot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("mybot.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("mybot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("mybot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("mybot.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("mybot.cli.commands._make_provider", return_value=object()), \
         patch("mybot.cli.commands._print_agent_response") as mock_print_response, \
         patch("mybot.bus.queue.MessageBus"), \
         patch("mybot.cron.service.CronService") as mock_cron_cls, \
         patch("mybot.agent.loop.AgentLoop") as mock_agent_loop_cls:
        cron_instance = MagicMock()
        cron_instance.start = AsyncMock(return_value=None)
        cron_instance.stop = MagicMock()
        mock_cron_cls.return_value = cron_instance
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "mybot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("mybot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mybot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("mybot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mybot.bus.queue.MessageBus", lambda: object())
    ...
    class _FakeCron:
        ...
        def register_system_job(self, _job):
            return None
    monkeypatch.setattr("mybot.cron.service.CronService", lambda _store: _FakeCron())
    ...
    monkeypatch.setattr("mybot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mybot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("mybot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("mybot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mybot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("mybot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mybot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("mybot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("mybot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mybot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("mybot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mybot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("mybot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("mybot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "mybot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("mybot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mybot.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "mybot.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "mybot.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("mybot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("mybot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("mybot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("mybot.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("mybot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mybot.api.server.create_app", _fake_create_app)
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from mybot.cli.commands import _migrate_cron_store
    ...
    with patch("mybot.config.paths.get_cron_dir", return_value=legacy_dir):
