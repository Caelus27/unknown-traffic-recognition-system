...
from mybot.config.paths import get_workspace_path, is_default_workspace
from mybot.config.schema import Config
from mybot.utils.helpers import sync_workspace_templates
from mybot.utils.restart import (
...
def cli() -> None:
    """Console-script entry point for installed `mybot` commands."""
    app()
...
def onboard(
    ...
    from mybot.config.loader import get_config_path, load_config, save_config, set_config_path
    from mybot.config.schema import Config
    ...
    if wizard:
        from mybot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'mybot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'mybot agent -m "Hello!"'
    serve_cmd = "mybot serve"
    if config:
    ...
    if wizard:
        ...
        console.print(f"  2. Start API server: [cyan]{serve_cmd}[/cyan]")
    else:
        console.print(f"  1. Add your API key to [cyan]{config_path}[/cyan]")
        console.print("     Get one at: https://openrouter.ai/keys")
        console.print(f"  2. Chat: [cyan]{agent_cmd}[/cyan]")
        console.print(f"  3. (Optional) Start API server: [cyan]{serve_cmd}[/cyan]")
...
def _make_provider(config: Config):
    ...
    if backend == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.mybot/config.json under providers.azure_openai section")
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from mybot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from mybot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show mybot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'mybot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger
    from mybot.agent.loop import AgentLoop
    from mybot.api.server import create_app
...
def agent(
    ...
    async def on_cron_job(job: "CronJob") -> str | None:
        """Execute a cron job through the agent loop."""
        if job.name == "dream":
            try:
        ...
        from mybot.utils.evaluator import evaluate_response
        ...
        if job.payload.deliver and job.payload.to and response:
            should_notify = await evaluate_response(
                response, reminder_note, provider, agent_loop.model,
            )
            if should_notify:
    ...
    from mybot.cron.types import CronJob, CronPayload
    ...
    if message:
        ...
        # Interactive mode — route through the bus for async delivery
...
def _login_github_copilot() -> None:
    try:
        from mybot.providers.github_copilot_provider import login_github_copilot
